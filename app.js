...
// ---------- Welcome splash ----------
// The branded front door — sync.js shows it at every sign-in after the cloud
// pull; ✨ Tour reopens it. Dense, one screen, no scrolling.

function showWelcomeSplash() {
  const tonightEl = document.getElementById("ws-tonight");
  ...
  if (entry && entry.freeDay) {
    ...
    if (r) { tonightEl.textContent = `Tonight: ${r.emoji} ${entry.customName || r.name}`; tonightEl.classList.remove("hidden"); }
    else tonightEl.classList.add("hidden");
  } else {
...
function hideWelcomeSplash() { document.getElementById("welcome-splash").classList.add("hidden"); }
